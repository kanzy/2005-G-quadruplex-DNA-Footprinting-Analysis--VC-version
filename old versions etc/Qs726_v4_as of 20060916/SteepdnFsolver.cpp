...
void CSteepdnFsolver::CalR7Mode(double P1, double P2, double P3, double P4, double P5, double P6)
{
	...
	double* x;
	x=new double[7];
	x[0]=p; x[1]=R; x[2]=Q1; x[3]=Q2; x[4]=Q3; x[5]=Q4; x[6]=Q9nt;
...
void CSteepdnFsolver::CalR6Mode(double P1,double P2,double P3,double P4,double P5)
{
	...
	double* x;
	x=new double[6];
	x[0]=p; x[1]=R; x[2]=Q1; x[3]=Q2; x[4]=Q3; x[5]=Q9nt;
	int i,j,k,maxiter=10000;
	double alpha,gtg,dfgtdfg,normf0,normf1;
...
void CSteepdnFsolver::CalR5Mode(double P1,double P2,double P3,double P4)
{
	...
	double* x;
	x=new double[5];
	x[0]=p; x[1]=R; x[2]=Q1; x[3]=Q2; x[4]=Q9nt;
	int i,j,k,maxiter=10000;
	double alpha,gtg,dfgtdfg,normf0,normf1;
...
{
	f[0]=(x[1]+x[3])*x[0]-P1;
	f[1]=x[1]*(1-x[0])*x[0]-P2 +x[0]*x[4]/3;
	f[2]=x[1]*(1-x[0])*(1-x[0])*x[0]-P3 +x[0]*x[4]/3;
	f[3]=x[1]*(1-x[0])*(1-x[0])*(1-x[0])*x[0]-P4 +x[0]*x[4]/3;
	f[4]=x[3]+x[2]+x[1]-1 +x[4];
}

void CSteepdnFsolver::ff6(double x[], double f[], double P1, double P2, double P3, double P4, double P5)
{
	f[0]=(x[1]+x[3]+x[4])*x[0]-P1 +x[0]*x[5]/2;
	f[1]=(x[1]+x[4])*(1-x[0])*x[0]-P2 +x[0]*x[5]/6;
	f[2]=x[1]*(1-x[0])*(1-x[0])*x[0]-P3 +x[0]*x[5]/3;
	f[3]=x[1]*(1-x[0])*(1-x[0])*(1-x[0])*x[0]-P4 +(2*x[0]-x[0]*x[0])*x[5]/6;
	f[4]=x[1]*(1-x[0])*(1-x[0])*(1-x[0])*(1-x[0])*x[0]+x[2]*x[0]-P5 +(x[0]-x[0]*x[0])*x[5]/6;
	f[5]=x[4]+x[3]+x[2]+x[1]-1 +x[5];

}

void CSteepdnFsolver::ff7(double x[], double f[], double P1, double P2, double P3, double P4, double P5, double P6)
{
	f[0]=(x[1]+x[3]+x[4]+x[5])*x[0]-P1 +x[0]*x[6]*6/9;
	f[1]=(x[1]+x[4]+x[5])*(1-x[0])*x[0]-P2 +x[0]*(1-x[0])*x[6]/3+x[0]*x[6]/9;
	f[2]=(x[1]+x[5])*(1-x[0])*(1-x[0])*x[0]-P3 +(x[0]*(1-x[0])+x[0])*x[6]/9;
	f[3]=x[1]*(1-x[0])*(1-x[0])*(1-x[0])*x[0]-P4 +(x[0]*x[0]*x[0]-3*x[0]*x[0]+3*x[0])*x[6]/9;
	f[4]=x[1]*(1-x[0])*(1-x[0])*(1-x[0])*(1-x[0])*x[0]+x[2]*x[0]-P5 +(x[0]*x[0]*x[0]-3*x[0]*x[0]+2*x[0])*x[6]/9;
	f[5]=x[1]*(1-x[0])*(1-x[0])*(1-x[0])*(1-x[0])*(1-x[0])*x[0]+(x[2]+x[3])*(1-x[0])*x[0]-P6 +(x[0]*x[0]*x[0]-5*x[0]*x[0]+4*x[0])*x[6]/9;
	f[6]=x[5]+x[4]+x[3]+x[2]+x[1]-1 +x[6];
}
...
void CSteepdnFsolver::fd5(double x[], double df[][5])
{
	...
	df[1][0]=x[1]*(1-2*x[0]) +x[4]/3;
	df[1][1]=(1-x[0])*x[0];
	df[1][2]=0;
	df[1][3]=0;
	df[1][4]=x[0]/3;

	df[2][0]=x[1]*(1-4*x[0]+3*x[0]*x[0]) +x[4]/3;
	df[2][1]=(1-x[0])*(1-x[0])*x[0];
	df[2][2]=0;
	df[2][3]=0;
	df[2][4]=x[0]/3;

	df[3][0]=x[1]*(1-6*x[0]+9*x[0]*x[0]-4*x[0]*x[0]*x[0]) +x[4]/3;
...
void CSteepdnFsolver::fd6(double x[], double df[][6])
{
	...
	df[2][0]=x[1]*(1-4*x[0]+3*x[0]*x[0]) +x[5]/3;
	df[2][1]=(1-x[0])*(1-x[0])*x[0];
	df[2][2]=0;
	df[2][3]=0;
	df[2][4]=0;
	...
	df[3][0]=x[1]*(1-6*x[0]+9*x[0]*x[0]-4*x[0]*x[0]*x[0]) +(2-2*x[0])*x[5]/6;
	df[3][1]=(1-x[0])*(1-x[0])*(1-x[0])*x[0];
	df[3][2]=0;
	df[3][3]=0;
	df[3][4]=0;
	df[3][5]=(2*x[0]-x[0]*x[0])/6;

	df[4][0]=x[1]*(1-x[0])*(1-x[0])*(1-x[0])*(1-5*x[0])+x[2] +(1-2*x[0])*x[5]/6;
	df[4][1]=(1-x[0])*(1-x[0])*(1-x[0])*(1-x[0])*x[0];
	df[4][2]=x[0];
	df[4][3]=0;
	df[4][4]=0;
	df[4][5]=(x[0]-x[0]*x[0])/6;
...
{
	df[0][0]=x[1]+x[3]+x[4]+x[5] +x[6]*6/9;
	df[0][1]=x[0];
	df[0][2]=0;
	df[0][3]=x[0];
	df[0][4]=x[0];
	...
	df[1][0]=(x[1]+x[4]+x[5])*(1-2*x[0]) +(1-2*x[0])*x[6]/3+x[6]/9;
	df[1][1]=(1-x[0])*x[0];
	df[1][2]=0;
	df[1][3]=0;
	df[1][4]=(1-x[0])*x[0];
	df[1][5]=(1-x[0])*x[0];
	df[1][6]=x[0]*(1-2*x[0])/3+x[0]/9;

	df[2][0]=(x[1]+x[5])*(1-4*x[0]+3*x[0]*x[0]) +(2-2*x[0])*x[6]/9;
	df[2][1]=(1-x[0])*(1-x[0])*x[0];
	df[2][2]=0;
	df[2][3]=0;
	df[2][4]=0;
	df[2][5]=(1-x[0])*(1-x[0])*x[0];
	df[2][6]=(x[0]*(1-x[0])+x[0])/9;

	df[3][0]=x[1]*(1-6*x[0]+9*x[0]*x[0]-4*x[0]*x[0]*x[0]) +(3*x[0]*x[0]-6*x[0]+3)*x[6]/9;
	df[3][1]=(1-x[0])*(1-x[0])*(1-x[0])*x[0];
	df[3][2]=0;
	df[3][3]=0;
	df[3][4]=0;
	df[3][5]=0;
	df[3][6]=(x[0]*x[0]*x[0]-3*x[0]*x[0]+3*x[0])/9;

	df[4][0]=x[1]*(1-x[0])*(1-x[0])*(1-x[0])*(1-5*x[0])+x[2] +(3*x[0]*x[0]-6*x[0]+2)*x[6]/9;
	df[4][1]=(1-x[0])*(1-x[0])*(1-x[0])*(1-x[0])*x[0];
	df[4][2]=x[0];
	df[4][3]=0;
	df[4][4]=0;
	df[4][5]=0;
	df[4][6]=(x[0]*x[0]*x[0]-3*x[0]*x[0]+2*x[0])/9;

	df[5][0]=x[1]*(1-x[0])*(1-x[0])*(1-x[0])*(1-x[0])*(1-6*x[0])+(x[2]+x[3])*(1-2*x[0]) +(3*x[0]*x[0]-10*x[0]+4)*x[6]/9;;
	df[5][1]=(1-x[0])*(1-x[0])*(1-x[0])*(1-x[0])*(1-x[0])*x[0];
	df[5][2]=(1-x[0])*x[0];
	df[5][3]=(1-x[0])*x[0];
	df[5][4]=0;
	df[5][5]=0;
	df[5][6]=(x[0]*x[0]*x[0]-5*x[0]*x[0]+4*x[0])/9;
